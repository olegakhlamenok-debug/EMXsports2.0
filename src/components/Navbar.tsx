    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16">
          {/* Logo — gradient text matching emxsports.com */}
          <Link
            href="/"
            className="font-heading font-medium text-sm uppercase tracking-[0.15em]"
            style={{
          ...
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="font-heading font-medium tracking-[0.15em] text-sm text-gray-500 hover:text-white uppercase transition-colors duration-300"
              >
                {link.label}
              </Link>
            ))}
